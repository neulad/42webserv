...
If the open operation fails, it throws an HttpError with a message indicating
that the file couldn't be created. It returns the file descriptor of the opened
file if successful.
...
If the fork fails, it throws an HttpError with a message indicating that the
child process couldn't be created. It returns the process ID of the child
...
It finds the first occurrence of '?' in the URI and extracts the substring from
that position to the end of the string.
...
It finds the first occurrence of '/' and '?' in the URI and extracts the
...
This function takes a filename as input and reads the contents of the file into
a string. It opens the file, reads its contents, and returns the string
representation of the file's contents. If the file cannot be opened, it throws a
runtime_error with a message indicating the failure.
...
This function takes a response string and a reference to an http::Response
object. It extracts the key and value from the response string and sets them as
headers in the http::Response object. If the key-value pair is not found, it
throws a runtime_error with a message indicating the failure.
...
void setResHeader(std::string response, http::Response &res) {
  ...
  if (pos <= 0)
    throw std::runtime_error("Failed to find key in header");
...
This function takes a response string and a reference to an http::Response
object. It extracts the body from the response string and sets it in the
http::Response object. It also calculates the content length of the body and
sets it as a header in the http::Response object. If the body is not found, it
throws a runtime_error with a message indicating the failure.
...
It checks the file extension and returns the corresponding interpreter from the
map. If the extension is not found in the map, it returns an empty string.
...
std::string getInterpreter(const std::string &path,
                           const std::map<std::string, std::string> &extMap) {
  const char *ext = &path[path.find_last_of('.')];
  std::map<std::string, std::string>::const_iterator it =
      extMap.find((std::string)ext);
  if (it != extMap.end()) {
    return it->second;
  }
  return "";
...
It splits the string into tokens based on the delimiter and returns a vector of
strings. It uses a stringstream to read the string and extract tokens until the
